...
#define MAX_FRAMES 12 // -Número total de 9 frames
...
int main()
{
	...
	Model Goku((char*)"Models/GOKU/cuerpo.obj");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::rotate(model, glm::radians(-rotRodDer), glm::vec3(1.0f, 0.0f, 0.0f)); //para usar los frames 
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		...
		model = glm::translate(model, glm::vec3(posX, posY, posZ));
		model = glm::rotate(model, glm::radians(rot), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::rotate(model, glm::radians(-rotBraDer), glm::vec3(0.5f, 1.0f, 0.0f));
		...
		//Animacion goku 
		//model = glm::translate(glm::mat4(1.0f), glm::vec3(245.0f, 400.0f, 400.0f)); //Ubicacion de la nave en el mapa
		//view = camera.GetViewMatrix();
